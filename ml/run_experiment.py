...
def run_arm(arm: str, seed: int, epochs: int, num_rounds: int, local_epochs: int) -> dict:
    set_seed(seed)
    print(f"\n=== {arm} (seed={seed}) ===")
    if arm == "isolated_real":
        return run_isolated(augmented=False, seed=seed, epochs=epochs)
    if arm == "isolated_augmented":
        return run_isolated(augmented=True, seed=seed, epochs=epochs)
    if arm == "centralized_real":
        return run_centralized(augmented=False, seed=seed, epochs=epochs)
    if arm == "centralized_augmented":
        return run_centralized(augmented=True, seed=seed, epochs=epochs)
    if arm == "federated_real":
        return run_federated(augmented=False, seed=seed, num_rounds=num_rounds, local_epochs=local_epochs)
    if arm == "federated_augmented":
        return run_federated(augmented=True, seed=seed, num_rounds=num_rounds, local_epochs=local_epochs)
